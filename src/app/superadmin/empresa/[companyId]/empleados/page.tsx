...
export default function CompanyEmployeesPage() {
  ...
  const fetchEmployees = useCallback(async () => {
    setLoading(true);
    const res = await fetch(`/api/admin/employees?companyId=${companyId}`);
  ...
  return (
    <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 py-8 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowCreate(true)} className="btn-primary">+ Nuevo empleado</button>
      </div>
              ...
              <tbody className="divide-y">
                {filtered.map((emp) => (
                  <tr key={emp.id} className="hover:bg-gray-50 dark:hover:bg-slate-700/30">
                    <td className="px-4 py-3 font-medium">{emp.surname ? `${emp.surname}, ${emp.name}` : emp.name}</td>
...
function EditEmployeeModal({ employee, onClose, onSuccess }: { employee: Employee; onClose: () => void; onSuccess: () => void }) {
  ...
  const [form, setForm] = useState({
    username: employee.username, name: employee.name, surname: employee.surname ?? "",
    role: employee.role as Role, department: employee.department ?? "",
    position: employee.position ?? "", nss: employee.nss ?? "",
    weeklyHours: employee.weeklyHours, password: "",
  });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const body: Record<string, unknown> = {
        ...
        nss: form.nss || null, weeklyHours: form.weeklyHours,
      };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
        <div className="p-6 border-b">
          ...
          <p className="text-sm text-gray-500 mt-1">{employee.name} {employee.surname}</p>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
          <div>
            <label className="label">Usuario <span className="text-danger-500">*</span></label>
            <input type="text" className="input font-mono uppercase" value={form.username}
              onChange={(e) => setForm({ ...form, username: e.target.value.toUpperCase().replace(/\s/g, "") })} required />
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="text" className="input" value={form.surname} onChange={(e) => setForm({ ...form, surname: e.target.value })} />
            </div>
          </div>
          <div>
            <label className="label">Nº Seguridad Social</label>
            <input type="text" className="input font-mono" value={form.nss} onChange={(e) => setForm({ ...form, nss: e.target.value })} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
            ...
            <div>
              ...
              <input type="text" className="input" value={form.position} onChange={(e) => setForm({ ...form, position: e.target.value })} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
          ...
          <div>
            ...
            <input type="password" className="input" placeholder="Dejar en blanco para no cambiar"
